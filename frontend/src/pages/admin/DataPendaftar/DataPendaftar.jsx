...
// O4/A1: Hanya menampilkan draft, menunggu, perbaikan
const STATUS_LABEL = {
  draft:     'Belum Mengisi',
  menunggu:  'Menunggu',
  perbaikan: 'Perbaikan',
    ...
    <DashboardLayout role={user?.role || 'operator'} user={userObj} activePath="/admin/pendaftar">
      <div className="mb-5">
        ...
        <p className="text-[12px] text-n500 mt-0.5">
          Menampilkan pendaftar dengan status: Draft, Menunggu, dan Perbaikan
        </p>
      </div>
      ...
      <div className="flex gap-3 mb-4">
        ...
        {/* O4/A1: Filter hanya status yang relevan */}
        <select
          value={filterStatus}
          onChange={e => { setFilterStatus(e.target.value); setPage(1); fetchData({ status: e.target.value, page: 1 }) }}
            ...
            {data.length === 0 ? (
              ...
              data.map((d, i) => (
                <Tr key={d.id}>
                  ...
                  <Td><Badge variant={d.status}>{STATUS_LABEL[d.status] ?? d.status}</Badge></Td>
                  <Td>
                    <Button size="xs" variant="primary" onClick={() => navigate(`/admin/pendaftar/${d.id}`)}>
                      Verifikasi
                    </Button>
                  </Td>
                </Tr>
